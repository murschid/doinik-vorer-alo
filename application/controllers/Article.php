...
class Article extends CI_Controller {
    ...
    public function index() {
        header('refresh:1800');
        $data['title'] = lang('ecn') . ' | ' . lang('feature');
        $data['home'] = array('main' => 'active');
        $wherein = array('national', 'economics', 'country', 'politics');
        $whereinent = array('entertainment', 'sports');
        $data['nationals'] = $this->external_model->wherein('tb_posts', NULL, 8, 'DESC', array('active' => 1), $wherein);
        $data['entertainments'] = $this->external_model->wherein('tb_posts', NULL, 7, 'DESC', array('active' => 1), $whereinent);
        $data['posts'] = $this->external_model->exquery('tb_posts', NULL, 6, 'DESC', array('active' => 1));
        $data['internationals'] = $this->external_model->exquery('tb_posts', NULL, 6, 'DESC', array('active' => 1, 'category' => 'international'));
    ...
    public function bangladesh() {
        ...
        $wherein = array('national', 'economics', 'country', 'politics');
        $data['posts'] = $this->external_model->wherein('tb_posts', NULL, 20, 'DESC', array('active' => 1), $wherein);
        $data['intro'] = $this->external_model->exsquery('tb_posts', array('active' => 1, 'intro' => 1));
        $data['view'] = 'external/category';
        $data['sidebar'] = 'external/sidebars/sidebar-long';
        $this->load->view('external/index', $data);
    }
    ...
    public function category() {
        ...
        $data['posts'] = $this->external_model->exquery('tb_posts', 0, 20, 'DESC', array('active' => 1, 'category' => $cat));
